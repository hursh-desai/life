      ...
      <CardContent className="p-6 grid md:grid-cols-3 gap-6">
        <div className="space-y-4 md:col-span-2">
          <div className="grid md:grid-cols-3 gap-4 items-end">
            ...
            <Tabs value={mode} onValueChange={(v)=>setMode(v as any)} className="md:col-span-2">
              ...
              <TabsContent value="lifespan" className="pt-4">
                <div className="grid grid-cols-[1fr_auto] gap-4 items-center">
                  <div className="px-2">
                    <Slider min={30} max={120} step={1} value={[lifespan]} onValueChange={(v)=>setLifespan(v[0])}/>
                  </div>
                  <Input type="number" className="w-24" value={lifespan} onChange={e=>setLifespan(parseInt(e.target.value || "0"))}/>
                </div>
              </TabsContent>
              <TabsContent value="deathdate" className="pt-4">
          ...
          <div className="flex items-center gap-6 pt-2">
            ...
            <div className="ml-auto text-sm text-slate-600 flex items-center gap-2"><Clock className="w-4 h-4"/> Today: {fmt(today)} · Age {Math.max(0, Math.min(150, yearsBetween(birthDate, today))).toFixed(1)}</div>
          </div>
        </div>

        <div className="rounded-xl bg-slate-50 border border-slate-200 p-4 space-y-2">
          <div className="text-sm text-slate-600">Your personalized window</div>
          <div className="text-lg font-semibold">{fmt(birthDate)} → {fmt(deathDate)}</div>
          ...
          <div className="text-xs text-slate-500">These are coarse heuristics; you’ll be able to customize and add your own overlays in a later iteration.</div>
        </div>
      </CardContent>
    </Card>
  );
